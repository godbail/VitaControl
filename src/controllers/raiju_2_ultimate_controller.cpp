...
#include "raiju_2_ultimate_controller.h"
#include "../logPlus.h"
...
static uint8_t buffer4Rajiu[0x100] = {0};
...
void Raiju2UltimateController::processReport(uint8_t *buffer, size_t length)
{
    ...
    Raiju2UltimateReport0x01 *report = (Raiju2UltimateReport0x01 *)buffer;
    ...
    if (report->cross)
        controlData.buttons |= SCE_CTRL_CROSS;
    if (report->circle)
        controlData.buttons |= SCE_CTRL_CIRCLE;
    if (report->triangle)
        controlData.buttons |= SCE_CTRL_TRIANGLE;
    if (report->square)
        controlData.buttons |= SCE_CTRL_SQUARE;
    ...
    if (report->r3)
        controlData.buttons |= SCE_CTRL_R3;

    // Map the menu buttons
    if (report->options)
        controlData.buttons |= SCE_CTRL_START;
    if (report->share)
        controlData.buttons |= SCE_CTRL_SELECT;
    if (report->ps)
